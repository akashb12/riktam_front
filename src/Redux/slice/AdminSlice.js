...
export const registerUser = createAsyncThunk('register', async(data) => {
    const res = await axiosRequest.post('admin/register',data).then((res)=>res).catch((err) => err.response);
...
export const getAllUsers = createAsyncThunk('getAllUsers',async() => {
    const res = await axiosRequest.get('/admin/getAllUsers').then((res)=>res).catch((err) => err.response);
    return {status:res.status,data:res.data};
})
